...
and notes, all stored as ordinary `VTODO`s that other clients can read.
On top of that:
...
No notifications, no streaks, no nagging. No account with me — you sign in
with your existing CalDAV credentials and your todos stay on your server.
...
so this is what that looks like when the bar doesn't move. Full type
...
configure and nothing to back up — Fold keeps no database.
...
connection needs to be private — and because Fold marks the cookie
`Secure`, browsers silently drop it over plain HTTP: sign-in appears to
work, then bounces you back to the login screen.
...
[Radicale](https://radicale.org) for trying Fold out — local testing only,
not your real server. See
[docs/user/local-caldav-server.md](docs/user/local-caldav-server.md).
