 ...
 */
const LISTS = [
  { name: 'Work', color: '#4F9D69FF', order: 1 },
...
const TODOS = {
  Work: [
    {
      summary: 'Prepare the sprint review',
      priority: 'high',
      due: { kind: 'utc', at: shift(0, 11) },
    },
    {
      summary: 'Update the release checklist',
      priority: 'high',
      due: { kind: 'utc', at: shift(0, 15) },
    },
    {
      summary: 'Write up the meeting notes',
      description: 'Circulate before the end of the week.',
      priority: 'medium',
      due: { kind: 'floating', at: shift(1, 12) },
    },
    {
      summary: 'Book a room for the workshop',
      description: 'Needs to seat eight.',
      priority: 'low',
      due: { kind: 'utc', at: shift(1, 16) },
    },
    // Overdue, so the clock pill and the danger ink have a subject.
    {
      summary: 'Review the open pull request',
      description: 'https://github.com/example/example/pull/9',
  ...
  Reading: [
    { summary: 'The one that was recommended' },
  ...
  Therapy: [
    // A zoned DUE, so the TZID round-trip has a subject.
    {
      summary: 'Appointment',
...
const DONE = [
  ...
  { list: 'Work', summary: 'Send the insurance form', daysAgo: 31 },
  ...
  { list: 'Work', summary: 'Ship the release notes', daysAgo: 1 },
