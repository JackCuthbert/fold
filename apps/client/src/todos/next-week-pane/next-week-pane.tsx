...
import { groupByDueDay, selectNextWeek, sortByDueInstant } from '../lib/today'
...
export function NextWeekPane(props: NextWeekPaneProps) {
  ...
  const days = groupByDueDay(due)

  return (
    <div className={paneStyles['pane']}>
      {/* docs/specs/next-7-days-view.md — no "Add a todo" row and no
          empty-state copy. An empty week is an ordinary state, and the
          title, the count line and the badge beside the title already say
          so between them.

          **Empty days are omitted**, not drawn as headings with nothing
          under them — `groupByDueDay` only ever yields days that have work.
          A fixed seven-day skeleton was the alternative and is argued down
          in the spec. */}
      {days.map((group) => (
        <DaySection
          key={group.day}
